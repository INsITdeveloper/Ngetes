...
export default async function handler(
  ...
  try {
    // Pastikan prompt diterima dengan benar
    const prompt = req.query.prompt ? String(req.query.prompt) : "A young man wearing a cool black hoodie in a cyberpunk style";
    ...
    const createTaskResponse = await axios.post(
      'https://api-preview.chatgot.io/api/v1/deepimg/flux-dev-lora',
      {
        device_id: deviceId,
        prompt: prompt, // Gunakan prompt yang sudah divalidasi
        n: "1",
        size: "1x1",
        output_format: "png",
      },
      {
        headers: {
          ...
          'User-Agent':
            'Mozilla/5.0 (Linux; Android 12; CPH2043) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36',
        },
      }
    );
    ...
    // Kurangi iterasi loop dari 10 menjadi 5 untuk menghindari timeout
    for (let i = 0; i < 5; i++) {
